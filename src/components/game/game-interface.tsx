...
interface GameInterfaceProps {
  ...
  onResolve: (debateHistory: string[]) => void;
  initialDebateHistory: string[];
}
...
export default function GameInterface({
  ...
  useEffect(() => {
    // Initialize the debate feed
    if (initialDebateHistory.length <= 1) {
       setDebate([
        {
          speaker: "Moderator",
  ...
  const handleNewTranscript = async (transcript: string) => {
    if (transcript.trim()) {
      ...
      if (result.success && result.data) {
        ...
        setDebate(debate); // Revert if AI fails
      }
  ...
  const handleResolve = () => {
    const finalDebateHistory = debate.map(
      (msg) => `${msg.speaker}: ${msg.message}`
    );
    onResolve(finalDebateHistory);
  };
      ...
      <footer className="mt-6 flex-shrink-0">
        ...
        <div className="text-center mt-4">
          ...
          <p className="text-xs text-muted-foreground mt-2">
            When your team has reached a consensus, resolve the dilemma.
          </p>
        </div>
      </footer>
    </div>
